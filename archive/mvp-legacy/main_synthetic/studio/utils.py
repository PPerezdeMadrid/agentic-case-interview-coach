...
def append_focus_areas(existing: list[str], new_values: list[str]) -> list[str]:
...
def compute_final_score(case_performance: dict, quality_dialog: dict) -> int:
    candidate_values = []
    for group in (case_performance, quality_dialog):
        for item in group.values():
            if not isinstance(item, dict):
                continue
            score = item.get("score")
            if isinstance(score, int):
                candidate_values.append(score)
    if not candidate_values:
        return 0
    return int(round(mean(candidate_values)))
