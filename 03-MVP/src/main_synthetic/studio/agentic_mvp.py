...
def _format_candidate_persona(scenario_data: dict) -> str:
    candidate_profile = scenario_data.get("candidate_profile", {})
...
def build_initial_interview_state(
    ...
    return {
        ...
        "scenario_ref": selected_ref or str(bundle["scenario"].get("scenario_id", "")),
        "scenario_data": bundle["scenario"],
...
def retrieve_case_node(state: InterviewState) -> InterviewState:
    scenario_ref = state.get("scenario_ref")
    ...
    if (
        ...
        and scenario_data
        and isinstance(case_data, dict)
        and case_data
        and isinstance(rubric_data, dict)
        and rubric_data
    ):
        return {}

    bundle = load_selected_simulation_bundle(scenario_ref=scenario_ref)
    return {
        "scenario_ref": scenario_ref or str(bundle["scenario"].get("scenario_id", "")),
        "scenario_data": bundle["scenario"],
        "case_data": bundle["case"],
        "rubric_data": bundle["rubric"],
    }
...
def interviewer_node(state: InterviewState) -> InterviewState:
    ...
    if turn_index == 0 and not transcript:
        if (
            ...
            or not case_data
            or not isinstance(rubric_data, dict)
            or not rubric_data
            or not isinstance(scenario_data, dict)
            or not scenario_data
        ):
            bundle = load_selected_simulation_bundle(scenario_ref=scenario_ref)
            scenario_data = bundle["scenario"]
            case_data = bundle["case"]
...
def candidate_node(state: InterviewState) -> InterviewState:
    ...
    candidate_transcript = state.get("candidate_transcript", [])
    scenario_data = state.get("scenario_data", {})
    persona_block = _format_candidate_persona(scenario_data if isinstance(scenario_data, dict) else {})
...
def eval_case_performance_node(state: InterviewState) -> InterviewState:
    final_score = state.get("final_score", 0)
    judge_reason = state.get("judge_reason", "")
    transcript = state.get("transcript", [])

    summary = (
        f"Eval Case Performance: final_score={final_score}/5."
        + (f" Reason: {judge_reason}" if judge_reason else "")
    )
    return {
...
def eval_dialog_quality_node(state: InterviewState) -> InterviewState:
    focus_area = state.get("focus_area", "")
    transcript = state.get("transcript", [])

    summary = "Eval Dialog Quality: assessed clarity, coherence, responsiveness, and communication quality."
    if focus_area and focus_area != "none":
        summary += f" Final focus area: {focus_area}."
...
def give_feedback_node(state: InterviewState) -> InterviewState:
    latest_feedback = state.get("latest_feedback", "").strip()
    final_score = state.get("final_score", 0)
    transcript = state.get("transcript", [])

    if not latest_feedback:
        latest_feedback = (
            f"Final feedback generated from case-performance and dialog-quality evaluation. Score: {final_score}/5."
        )

    return {
        "latest_feedback": latest_feedback,
        "messages": [AIMessage(content=latest_feedback, name="feedback")],
        "transcript": transcript + [f"Give Feedback: {latest_feedback}"],
    }
...
agentic_02_builder = StateGraph(InterviewState)
